...
Перем тзОписаниеВыгружаемыхДанных; // Хранит набор выгружаемых ссылочных типов данных со списком реквизитов и таб. частей.
Перем МассивРезультат;

Перем ПараметрыЗаписиJSON;
Перем НастройкиСериализацииJSON;

Перем СтекОбъектыВОбработке; // Соответствие, хранит ссылки объектов ссылочного типа, для которых ещё активен процесс сериализации.
...
// ГЛАВНЫЙ ВХОД Сериализует переданное значение в структуру с простыми типами данных.
//
// Параметры:
...
//
Функция СериализоватьВПростыеТипы(Значение, Знач ТолькоСсылка = Истина, ЭтоСериализацияРеквизита = Ложь, Знач УровеньРекурсии = 1, Знач ИмяПКО = "") Экспорт
	...
	Если ЭтоПростойТип(Значение) Тогда
		...
		Если Значение = Неопределено Тогда
			НовоеЗначение = "_ТипНеопределено";
		Иначе
	...
	//Ищем описание выгружаемого объекта:
	Если НЕ мТипыПеречислений.СодержитТип(ТипЗначения) Тогда
		мОписаниеТипаДанных = ОписаниеВыгружаемыхДанныхПоТипу(ТипЗначения, Значение, ИмяПКО);
		Если мОписаниеТипаДанных = Неопределено Тогда
			Возврат "_ТипНеопределено";
		КонецЕсли;
	КонецЕсли;
	...
	Если Метаданные.Справочники.Содержит(МетаданныеЗначения) Тогда
		...
		Если ЭтоСериализацияРеквизита И ТолькоСсылка Тогда // Обход реквизитов основного объекта
			НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "СправочникСсылка", ТолькоСсылка, МетаданныеЗначения, 
			мОписаниеТипаДанных, УровеньРекурсии, ИмяПКО);
	...
	ИначеЕсли Метаданные.Документы.Содержит(МетаданныеЗначения) Тогда
		НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "ДокументСсылка", ТолькоСсылка, МетаданныеЗначения, 
		мОписаниеТипаДанных, УровеньРекурсии, ИмяПКО);
		
	ИначеЕсли Метаданные.Перечисления.Содержит(МетаданныеЗначения) Тогда
		НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "ПеречислениеСсылка", Истина, МетаданныеЗначения, Неопределено, УровеньРекурсии, ИмяПКО);
	...
	Если (НовоеЗначение <> "_ТипНеопределено" И НовоеЗначение <> Неопределено) 
		И (НЕ ТолькоСсылка) 
		И (НЕ мТипыПеречислений.СодержитТип(ТипЗначения)) Тогда
	...
	// удаляем ссылочное значение из стека объектов в обработке:
	Если НЕ ТолькоСсылка Тогда
		
		НомерУровняРекурсии = СтекОбъектыВОбработке.Получить(Значение);
		Если НомерУровняРекурсии <> Неопределено И НомерУровняРекурсии = УровеньРекурсии Тогда
			СтекОбъектыВОбработке.Удалить(Значение);
		КонецЕсли;
...
//
Функция СериализоватьПоСсылкеВСтруктуру(Знач ЗначениеСсылка, 
										КоллекцияТипа, 
										ТолькоСсылка = Ложь, 
										МетаданныеЗначения, 
										ОписаниеТипаДанных = Неопределено, 
										Знач УровеньРекурсии, 
										Знач ИмяПКО)
	...
	Если СсылкаСтруктурой = Неопределено Тогда
		...
		Если НЕ СтрНачинаетсяС(КоллекцияТипа, "ПеречислениеСсылка") Тогда
			ЗаполнитьСвойстваПоиска(ЗначениеСсылка, СсылкаСтруктурой, ОписаниеТипаДанных["_СвойстваПоиска"], УровеньРекурсии);
		КонецЕсли;
	...
	ИначеЕсли СтекОбъектыВОбработке.Получить(ЗначениеСсылка) = Неопределено Тогда 
		// сериализация объекта ещё не начиналась, добавляем ссылку в стек:
		СтекОбъектыВОбработке.Вставить(ЗначениеСсылка, УровеньРекурсии);
	ИначеЕсли СтекОбъектыВОбработке.Получить(ЗначениеСсылка) <> Неопределено Тогда
		// Сериализация объекта началась на каком-то из предыдущих уровней, прерываем процесс для избежания зацикливания.
		Возврат Неопределено;
	КонецЕсли;
	
	ЗначениеСтруктурой = Новый Структура;
	ЗначениеСтруктурой.Вставить("_ИмяПКО", ИмяПКО);
	...
	Если НЕ СтрНачинаетсяС(КоллекцияТипа, "ПеречислениеСсылка") Тогда
		...
		ВыгрузитьРеквизиты(ЗначениеСсылка, ЗначениеСтруктурой._Свойства, ОписаниеТипаДанных["_Свойства"], УровеньРекурсии);
		ВыгрузитьТабличныеЧасти(ЗначениеСсылка, МетаданныеЗначения, ЗначениеСтруктурой._ТабличныеЧасти, ОписаниеТипаДанных["_ТабличныеЧасти"], УровеньРекурсии);
...
//
Процедура ЗаполнитьСвойстваПоиска(ЗначениеСсылка, СсылкаСтруктурой, МассивСвойствПоиска, Знач УровеньРекурсии) 
	...
	Если МассивСвойствПоиска.Количество() > 0 Тогда
		ВыгрузитьРеквизиты(ЗначениеСсылка, СвойстваПоиска, МассивСвойствПоиска, УровеньРекурсии);
	КонецЕсли;
...
//   Неопределено
//
Функция ВыгрузитьРеквизиты(ЗначениеСсылка, СтруктураРеквизитов, МассивСвойств, Знач УровеньРекурсии);
	
	Для Каждого ИмяСвойства Из МассивСвойств Цикл
		
		Попытка
			ЗначениеРеквизита = ЗначениеСсылка[ИмяСвойства.Ключ];
			ЗначениеСвойства = СериализоватьВПростыеТипы(ЗначениеРеквизита, Истина, Истина, , ИмяСвойства.Значение); // Сначала сериализуем просто как ссылку
			СтруктураРеквизитов.Вставить(ИмяСвойства.Ключ, ЗначениеСвойства);
			
			// Повторно вызываем для полной сериализации:
			Если  НЕ мТипыПеречислений.СодержитТип(ТипЗнч(ЗначениеРеквизита)) Тогда
				СериализоватьВПростыеТипы(ЗначениеРеквизита, Ложь, Истина, УровеньРекурсии + 1, ИмяСвойства.Значение);
			КонецЕсли;
		Исключение
		КонецПопытки;
...
//   Неопределено
//
Функция ВыгрузитьТабличныеЧасти(ЗначениеСсылка, МетаданныеЗначения, СтруктураТабЧастей, ОписаниеСвойствТЧ, Знач УровеньРекурсии)
		...
		Для Каждого СтрокаТЧ Из ДанныеТЧ Цикл
			...
			Для Каждого СвойствоТЧ Из НаборСвойствТЧ Цикл
				...
				Попытка
					...
					ЗначениеСвойства = СериализоватьВПростыеТипы(ЗначениеРеквизита, Истина, Истина,, СвойствоТЧ.Значение);
				...
				Если НЕ мТипыПеречислений.СодержитТип(ТипЗнч(ЗначениеРеквизита)) Тогда
					СериализоватьВПростыеТипы(ЗначениеРеквизита, Ложь, Истина, УровеньРекурсии + 1, СвойствоТЧ.Значение);
				КонецЕсли;
			КонецЦикла;
...
// ОписаниеВыгружаемыхДанныхПоТипу.
//
// Параметры:
...
//
Функция ОписаниеВыгружаемыхДанныхПоТипу(ТипЗначения, Значение, ИмяПКО) 
	
	сОписание = Неопределено;
	
	//ИмяПКО = "";
	СтрокаОписание = тзОписаниеВыгружаемыхДанных.Найти(ТипЗначения, "ТипДанных");
	Если СтрокаОписание = Неопределено Тогда
		
		Сообщить(СтрШаблон("Для объекта типа ""%1"" отсутствет описание выгружаемых данных в настройка учетной записи обмена!", Строка(ТипЗначения)));
		
	Иначе
		
		сПравила = СтрокаОписание.Правила;
		Если СтрокаОписание.НесколькоПравил Тогда
			
			Если НЕ ПустаяСтрока(СтрокаОписание.АлгоритмВыборИмениПравила) Тогда
				
				Попытка
					
					Выполнить(СтрокаОписание.АлгоритмВыборИмениПравила);
			...
			Иначе
				...
				МассивИмен = СтрРазделить(СтрокаОписание.ИменаПравил, ",", Ложь);
		...
		Иначе
			...
			Если ПустаяСтрока(ИмяПКО) Тогда
				ИмяПКО = СтрокаОписание.ИмяПравилаПоУмолчанию;
			КонецЕсли;
			
		КонецЕсли;
		
		сОписание = сПравила[ИмяПКО];
		
	КонецЕсли;
	
	Если сОписание = Неопределено Тогда
		Сообщить(СтрШаблон("Для объекта типа ""%1"" отсутствет описание выгружаемых данных с именем ""%2"" в настройка учетной записи обмена!", 
		Строка(ТипЗначения), ИмяПКО));
	КонецЕсли;
	
	Возврат сОписание;
	
КонецФункции // ОписаниеВыгружаемыхДанныхПоТипу
...
Процедура ЗаполнитьОписаниеВыгружаемыхТиповДанных() Экспорт 
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОписаниеВыгружаемыхОбъектов.ТипОбъекта КАК ТипОбъекта,
	|	ОписаниеВыгружаемыхОбъектов.ОписаниеПравил КАК ОписаниеПравил,
	|	ОписаниеВыгружаемыхОбъектов.ИменаПравил КАК ИменаПравил,
	|	ОписаниеВыгружаемыхОбъектов.НесколькоПравил КАК НесколькоПравил,
	|	ОписаниеВыгружаемыхОбъектов.АлгоритмВыборИмениПравила КАК АлгоритмВыборИмениПравила,
	|	ОписаниеВыгружаемыхОбъектов.ИмяПравилаПоУмолчанию КАК ИмяПравилаПоУмолчанию
	|ИЗ
	|	Справочник.СМП_УчетныеЗаписиОбменов.ОписаниеВыгружаемыхОбъектов КАК ОписаниеВыгружаемыхОбъектов
	|ГДЕ
	|	ОписаниеВыгружаемыхОбъектов.Ссылка = &УчетнаяЗапись
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОписаниеВыгружаемыхОбъектов.НомерСтроки";
	Запрос.УстановитьПараметр("УчетнаяЗапись", УчетнаяЗаписьОбмена);
	
	тзОписаниеВыгружаемыхДанных = Запрос.Выполнить().Выгрузить();
	тзОписаниеВыгружаемыхДанных.Колонки.Добавить("ТипДанных", Новый ОписаниеТипов("Тип"));
	тзОписаниеВыгружаемыхДанных.Колонки.Добавить("Правила");
	
	Для Каждого СтрокаТЗ Из тзОписаниеВыгружаемыхДанных Цикл
		
		СтрокаТЗ.ТипДанных = Тип(СтрокаТЗ.ТипОбъекта);
		
		ЧтениеДж = Новый ЧтениеJSON;
		ЧтениеДж.УстановитьСтроку(СтрокаТЗ.ОписаниеПравил);
		сОписания = ПрочитатьJSON(ЧтениеДж, Истина);
		ЧтениеДж.Закрыть();
		
		СтрокаТЗ.Правила = сОписания;
		
	КонецЦикла;
	
	тзОписаниеВыгружаемыхДанных.Колонки.Удалить("ТипОбъекта");
	тзОписаниеВыгружаемыхДанных.Колонки.Удалить("ОписаниеПравил");
...
СтекОбъектыВОбработке = Новый Соответствие;
...
//сОписаниеВыгружаемыхДанных = Неопределено;
