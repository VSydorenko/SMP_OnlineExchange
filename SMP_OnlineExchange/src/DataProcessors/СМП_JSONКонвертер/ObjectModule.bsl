...
//
Функция СериализоватьВПростыеТипы(Значение, Знач ТолькоСсылка = Истина, ЭтоСериализацияРеквизита = Ложь, Знач УровеньРекурсии = 1) Экспорт
	...
	Если Метаданные.Справочники.Содержит(МетаданныеЗначения) Тогда
		...
		Если ЭтоСериализацияРеквизита И ТолькоСсылка Тогда // Обход реквизитов основного объекта
			НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "СправочникСсылка", ТолькоСсылка, МетаданныеЗначения, мОписаниеТипаДанных, УровеньРекурсии);
	...
	ИначеЕсли Метаданные.Документы.Содержит(МетаданныеЗначения) Тогда
		НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "ДокументСсылка", ТолькоСсылка, МетаданныеЗначения, мОписаниеТипаДанных, УровеньРекурсии);
		
	ИначеЕсли Метаданные.Перечисления.Содержит(МетаданныеЗначения) Тогда
		НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "ПеречислениеСсылка", Истина, МетаданныеЗначения, Неопределено, УровеньРекурсии);
	...
	Для Каждого ИмяСвойства Из МассивСвойств Цикл
		...
		Попытка
			...
			Если  НЕ мТипыПеречислений.СодержитТип(ТипЗнч(ЗначениеРеквизита)) Тогда
				СериализоватьВПростыеТипы(ЗначениеРеквизита, Ложь, Истина, УровеньРекурсии + 1);
			КонецЕсли;
		Исключение
		КонецПопытки;
