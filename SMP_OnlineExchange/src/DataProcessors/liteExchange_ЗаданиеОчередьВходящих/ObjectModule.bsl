...
Функция ОбработкаОчереди()
	...
	Запрос.УстановитьПараметр("Статус", Перечисления.СМП_СтатусыОбработкиСообщений.Обработано);
	Запрос.УстановитьПараметр("УникальныйИдентификатор", Идентификатор);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		СМП_ОнлайнОбменУправлениеОчередями.УстановитьСтатусВОбработке(Выборка.УникальныйИдентификатор, "леОчередьВходящихСообщений");
		
		Попытка
			Данные = леJSONКонвертер4.ПрочитатьJSON_Расширенная(Выборка.Данные);
		Исключение
			СМП_ОнлайнОбменУправлениеОчередями.УстановитьСтатусОшибкаОбработки(Выборка.УникальныйИдентификатор, "леОчередьВходящихСообщений", ОписаниеОшибки());
			Продолжить;
		КонецПопытки;
		...
		Попытка
			...
			Иначе
				...
				леJSONКонвертер4.ЗаписатьДанные();
			КонецЕсли;
			СМП_ОнлайнОбменУправлениеОчередями.УстановитьСтатусОбработано(Выборка.УникальныйИдентификатор, "леОчередьВходящихСообщений");
		Исключение
			СМП_ОнлайнОбменУправлениеОчередями.УстановитьСтатусОшибкаОбработки(Выборка.УникальныйИдентификатор, "леОчередьВходящихСообщений", ОписаниеОшибки());
		КонецПопытки;
