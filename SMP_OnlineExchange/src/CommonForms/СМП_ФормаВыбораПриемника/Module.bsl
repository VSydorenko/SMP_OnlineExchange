...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ИмяТабличнойчастиОбъекта", ИмяТабличнойЧастиОбъекта);
...
Процедура ДеревоОбъектовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные.Уровень = 2 Тогда
		...
		мДанные = Новый Структура("Имя, Вид, Тип");
		ЗаполнитьЗначенияСвойств(мДанные, Элемент.ТекущиеДанные);
		Закрыть(мДанные);
...
Процедура ЗаполнитьДеревоПоВидуОбъекта() 
	...
	ИначеЕсли ВидОбъектаОбмена = ПредопределенноеЗначение("Перечисление.СМП_ВидыОбъектовОбмена.ГруппаСвойствОбъекта") Тогда
		мДерево = ЗаполнитьДеревоТабличнымиЧастямиОбъектаМд();
	КонецЕсли;
...
Функция ЗаполнитьДеревоСвойствамиОбъектаМд() 
	...
	СтрокаОбъектМд.Имя = ИмяРодительскогоОбъекта + ?(Не ПустаяСтрока(ИмяТабличнойЧастиОбъекта), "." + ИмяТабличнойЧастиОбъекта, "");
	СтрокаОбъектМд.Уровень = 1;
	
	ТипРодителя = Тип(ИмяРодительскогоОбъекта);
	ОбъектМд = Метаданные.НайтиПоТипу(ТипРодителя);
	
	Если НЕ ПустаяСтрока(ИмяТабличнойЧастиОбъекта) Тогда
		ТЧ = ОбъектМд.ТабличныеЧасти[ИмяТабличнойЧастиОбъекта];
		СтрокаОбъектМд.Синоним = ТЧ.Синоним;
		Для Каждого тчРеквизит Из ТЧ.Реквизиты Цикл
			СтрокаРеквизит = СтрокаОбъектМд.Строки.Добавить();
			СтрокаРеквизит.Имя = тчРеквизит.Имя;
			СтрокаРеквизит.Синоним = тчРеквизит.Синоним;
			СтрокаРеквизит.Уровень = 2;
			СтрокаРеквизит.Вид = Перечисления.ВидыСвойств.Реквизит;
			СтрокаРеквизит.Тип = ПолучитьТипыСвойстваСтрокой(тчРеквизит.Тип);
...
// Возвращает дерево со списком табличных частей объекта метаданных.
//
// Возвращаемое значение:
//   ДеревоЗначений - см. ИнициализироватьДерево().
//
&НаСервере
Функция ЗаполнитьДеревоТабличнымиЧастямиОбъектаМд() 
	...
	Для Каждого мдТабЧасть Из КоллекцияТч Цикл
		СтрокаТч = СтрокаОбъект.Строки.Добавить();
		СтрокаТч.Имя = мдТабЧасть.Имя;
		СтрокаТч.Синоним = мдТабЧасть.Синоним;
		СтрокаТЧ.Вид = Перечисления.ВидыСвойств.ТабличнаяЧасть;
		СтрокаТч.Уровень = 2;
	КонецЦикла;
	
	Возврат мДерево;
	
КонецФункции // ЗаполнитьДеревоТабличнымиЧастямиОбъектаМд
...
Процедура НастроитьФормуПоВидуОбъектаОбмена() 
	...
	ИначеЕсли ВидОбъектаОбмена = ПредопределенноеЗначение("Перечисление.СМП_ВидыОбъектовОбмена.ГруппаСвойствОбъекта") Тогда
		ЭтотОбъект.Заголовок = "Табличные части объекта метаданных";
