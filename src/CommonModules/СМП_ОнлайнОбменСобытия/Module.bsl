...
Процедура ОтправитьСообщение(УчетнаяЗаписьОбмена, Сообщение) Экспорт
	...
	ПакетИсходящихДанных = СМП_СобытияОтправкиДанных.ЗаписатьДанныеВJSON_Расширенная(Сообщение);
	...
	Если ПараметрыТранспортногоКанала.СпособПодключения = Перечисления.СМП_ВидыТранспортовОбмена.Файл Тогда
		СМП_СобытияОтправкиДанных.СохранитьВКаталог(ПакетИсходящихДанных, ПараметрыТранспортногоКанала.ФайлКаталогОбмена, "УНФ", "БУ");
	КонецЕсли;
