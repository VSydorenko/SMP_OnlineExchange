...
Функция ПоместитьСообщениеВОчередьОбработки(Данные) Экспорт
	...
	Если леОбщегоНазначения.РежимОтладки() Тогда
		леУправлениеОчередями.ОбработкаОчередиДанных(УникальныйИдентификатор);
	Иначе
...
Функция ПоместитьСообщениеВОчередьИсходящих(Сообщение) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	УникальныйИдентификатор = Строка(Новый УникальныйИдентификатор);
	МенеджерЗаписи = ПолучитьМенеджерСообщения("леОчередьИсходящихСообщений", УникальныйИдентификатор);
	...
	Если леОбщегоНазначения.РежимОтладки() Тогда
		леУправлениеОчередями.ОтправитьСообщение(УникальныйИдентификатор);
	Иначе
		Параметры = Новый Массив;
		Параметры.Добавить(УникальныйИдентификатор);
		ФоновыеЗадания.Выполнить("леУправлениеОчередями.ОтправитьСообщение", Параметры);
	КонецЕсли;
...
Функция НаборЗаписейПоИмениОчереди(Очередь)
	...
	ИначеЕсли Найти(НРег(Очередь), "исход") Тогда
		НаборЗаписей = РегистрыСведений.леОчередьИсходящихСообщений.СоздатьНаборЗаписей();
...
Функция ОбработкаОчередиДанных(УникальныйИдентификатор) Экспорт
	...
	леПодключаемыеОбработчики.ВыполнитьПодключаемыйОбработчикВФоне(Данные);
...
Функция ПолучитьИсходящееСообщениеПоИдентификатору(УникальныйИдентификатор)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	леОчередьИсходящихСообщений.Данные КАК Сообщение
	|ИЗ
	|	РегистрСведений.леОчередьИсходящихСообщений КАК леОчередьИсходящихСообщений
	|ГДЕ
	|	леОчередьИсходящихСообщений.УникальныйИдентификатор = &УникальныйИдентификатор";
	Запрос.УстановитьПараметр("УникальныйИдентификатор",УникальныйИдентификатор);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
...
Функция Пауза(ИД, КоличествоСекунд = Неопределено) Экспорт	
	...
	Если ЗначениеЗаполнено(КоличествоСекунд) Тогда
		...
		ФоновыеЗадания.Выполнить("леУправлениеОчередями.Пауза", ПараметрыЗадания);
	КонецЕсли;
КонецФункции
#КонецОбласти