...
Функция ОтправитьОбъект1с(Данные, Адрес, Параметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Данные) = Тип("Строка") Тогда
		леJSONКонвертер4 = леПодключаемыеОбработчики.ПодключитьОбработку("леJSONКонвертер4");
		ДанныеJSONСтрокой = леJSONКонвертер4.ЗаписатьJSON_Расширенная(Данные);
	Иначе
		ДанныеJSONСтрокой = Данные;
	КонецЕсли;
...
Функция ОтправитьJSON(Данные, Адрес, Параметры = Неопределено) Экспорт
	...
	Возврат леHTTPRequest.ОтправитьДанные1сНаУдаленыйСервер(ДанныеJSONСтрокой, Адрес, Параметры);
