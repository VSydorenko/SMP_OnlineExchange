...
//
Функция СформироватьОписаниеПравилКонвертации(УчетнаяЗаписьОбмена) Экспорт
	...
	Если НЕ ПакетРезультатов[0].Пустой() И НЕ ПакетРезультатов[1].Пустой() Тогда
		...
		Пока ВыборкаПКО.Следующий() Цикл
			...
			Если сНаборРеквизитов.Количество() > 0 Тогда
...
КонецФункции // СформироватьОписаниеПравилКонвертации
...
Процедура ЗаполнитьНаборВыгружаемыхРеквизитов(ВыборкаСвойств, СсылкаПКО, НаборРеквизитов) 
	...
	Пока ВыборкаСвойств.НайтиСледующий(СсылкаПКО, "Владелец") Цикл
		...
		Иначе
			...
			ПоискПоРеквизиту = НаборРеквизитов.Получить(ИмяРеквизита);
			
			Если ПоискПоРеквизиту = Неопределено Тогда // Реквизит добавляется впервые
				
				ПоискПоРеквизиту = ВыборкаСвойств.Поиск;
				
			ИначеЕсли ВыборкаСвойств.Поиск И НЕ ПоискПоРеквизиту Тогда
				
				ПоискПоРеквизиту = ВыборкаСвойств.Поиск;
				
			КонецЕсли;
			
			НаборРеквизитов.Вставить(ИмяРеквизита, ПоискПоРеквизиту);
