...
Функция ОбработкаОчереди()	
	...
	Пока Выборка.Следующий() Цикл
		Данные = леУправлениеОчередями.ПолучитьДанныеПоИдентификатору(Выборка.УникальныйИдентификатор);		
		Ответ = леСобытия.ОтправитьДанные(Данные);
		Если Ответ = Неопределено Тогда
			леУправлениеОчередями.УстановитьСтатусОбработано(Выборка.УникальныйИдентификатор);
		Иначе
			леУправлениеОчередями.УстановитьСтатусОшибкаОбработки(Выборка.УникальныйИдентификатор, , Ответ);
		КонецЕсли;
	КонецЦикла;
КонецФункции
