...
Перем ОсновнойОбъектСсылка;     // Хранит основной объект для сериализации.
Перем РезультатКонвертации;     // Результирующая структура для преобразования в строку JSON.
...
Функция ЗаписатьДанныеВJSON(Данные, ТолькоСсылки = Истина, ПравилаКонвертации = Неопределено) Экспорт
	
	ОсновнойОбъектСсылка = Данные;
	...
	ЗаписатьJSON(ЗаписьJSON, Данные, НастройкиСериализацииJSON, "КонвертацияСложныхТиповДляJSON", ЭтотОбъект, ДопПараметры);
...
Функция КонвертацияСложныхТиповДляJSON(Свойство, Значение, ДопПараметры, Отказ) Экспорт
	...
	СериализоватьВПростыеТипы(Значение, ТолькоСсылки);
...
Функция СериализоватьВПростыеТипы(Значение, Знач ТолькоСсылка = Истина) Экспорт
	...
	ИначеЕсли Метаданные.Перечисления.Содержит(МетаданныеЗначения) Тогда
		НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "ПеречислениеСсылка", ТолькоСсылка, МетаданныеЗначения);
	...
	ИначеЕсли ЗначениеЗаполнено(Значение) 
		И НЕ ТолькоСсылка 
		И НЕ Перечисления.ТипВсеСсылки().СодержитТип(ТипЗначения) Тогда // Это сериализован реквизит основного объекта. Данные реквизита выгружаются полностью.
...
//
Функция СериализоватьПоСсылкеВСтруктуру(Знач ЗначениеСсылка, КоллекцияТипа, ТолькоСсылка = Ложь, МетаданныеЗначения)
	...
	ЗначениеСтруктурой = Новый Структура(мСтандартныеРеквизиты[КоллекцияТипа]);
	ЗаполнитьЗначенияСвойств(ЗначениеСтруктурой, ЗначениеСсылка);
	...
	ЗначениеСтруктурой.Вставить("_Свойства", Новый Структура); // Для реквизитов
	ЗначениеСтруктурой.Вставить("_ТабличныеЧасти", Новый Структура); // Для табличных частей
	
	Попытка
		рРодитель = МетаданныеЗначения.СтандартныеРеквизиты["Родитель"];
		ЗначениеРодитель = ЗначениеСсылка["Родитель"];
		ЗначениеСтруктурой._Свойства.Вставить("Родитель", СериализоватьСсылку(ЗначениеРодитель, ТипСтрокой, Гнпп));
		СериализоватьВПростыеТипы(ЗначениеРодитель,
			?(ЗначениеСсылка = ОсновнойОбъектСсылка, Ложь, Истина));
	Исключение
	КонецПопытки;
	
	Попытка
		рЭтоГруппа = МетаданныеЗначения.СтандартныеРеквизиты["ЭтоГруппа"];
		ЗначениеСтруктурой._Свойства.Вставить("ЭтоГруппа", ЗначениеСсылка["ЭтоГруппа"]);
	Исключение
	КонецПопытки;
	
	Попытка
		рВладелец = МетаданныеЗначения.СтандартныеРеквизиты["Владелец"];
		ЗначениеВладелец = ЗначениеСсылка["Владелец"];
		ЗначениеСтруктурой._Свойства.Добавить("Владелец", СериализоватьСсылку(ЗначениеВладелец, ТипСтрокой, Гнпп));
		СериализоватьВПростыеТипы(ЗначениеВладелец,
			?(ЗначениеСсылка = ОсновнойОбъектСсылка, Ложь, Истина));
	Исключение
	КонецПопытки;
	...
	Если НЕ СтрНачинаетсяС(КоллекцияТипа, "ПеречислениеСсылка") Тогда
		...
		ВыгрузитьРеквизиты(ЗначениеСсылка, МетаданныеЗначения, ЗначениеСтруктурой._Свойства);
		ВыгрузитьТабличныеЧасти(ЗначениеСсылка, МетаданныеЗначения, ЗначениеСтруктурой._ТабличныеЧасти);
...
//   Неопределено
//
Функция ВыгрузитьРеквизиты(ЗначениеСсылка, МетаданныеЗначения, СтруктураРеквизитов);
	
	мдРеквизиты = МетаданныеЗначения.Реквизиты;
	
	Для Каждого Реквизит Из мдРеквизиты Цикл
		
		// Для основного объекта выполняется полная сериализация реквизитов
		ЗначениеРеквизита = ЗначениеСсылка[Реквизит.Имя];
		СсылкаСтруктурой = СериализоватьВПростыеТипы(ЗначениеРеквизита);
		
		// Повторный вызов для полной сериализации реквизита основного объекта
		Если ЗначениеСсылка = ОсновнойОбъектСсылка Тогда
			СериализоватьВПростыеТипы(ЗначениеРеквизита, Ложь);
		КонецЕсли;
		
		СтруктураРеквизитов.Вставить(Реквизит.Имя, СсылкаСтруктурой);
...
//   Неопределено
//
Функция ВыгрузитьТабличныеЧасти(ЗначениеСсылка, МетаданныеЗначения, СтруктураТабЧастей)
	
	мдТабЧасти = МетаданныеЗначения.ТабличныеЧасти;
	Для Каждого МетаТЧ Из мдТабЧасти Цикл
		
		ДанныеТЧ = ЗначениеСсылка[МетаТЧ.Имя];
		КоличествоСтрок = ДанныеТЧ.Количество();
		Если КоличествоСтрок > 0 Тогда
			
			мдРеквизитыТЧ = МетаТЧ.Реквизиты;
			...
			Для Каждого СтрокаТЧ Из ДанныеТЧ Цикл
				...
				Для Каждого мдРеквизит Из мдРеквизитыТЧ Цикл
					
					ИмяРеквизита = мдРеквизит.Имя;
					ЗначениеРеквизита = СтрокаТЧ[ИмяРеквизита];
					СсылкаСтруктурой = СериализоватьВПростыеТипы(ЗначениеРеквизита);
					
					// Повторный вызов для полной сериализации реквизитов ТЧ основного объекта:
					Если ЗначениеСсылка = ОсновнойОбъектСсылка Тогда
						СериализоватьВПростыеТипы(ЗначениеРеквизита, Ложь);
					КонецЕсли;
					
					ДанныеСтрокиТЧ.Вставить(ИмяРеквизита, СсылкаСтруктурой);
...
Гнпп = 0;
РезультатКонвертации = Новый Структура("Объекты", Новый Массив);
