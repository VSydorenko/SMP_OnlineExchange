...
Перем СтекСсылкиКОбработке;     // Хранит все ссылки элементов справочников по уровням
Перем УровеньВложенности;
Перем СериализацияЗакончена;
...
Функция СериализоватьВПростыеТипы(Значение, Знач ТолькоСсылка = Истина, ЭтоСериализацияРеквизита = Ложь) Экспорт
	...
	Если Метаданные.Справочники.Содержит(МетаданныеЗначения) Тогда
		...
		Если Значение.Пустая() Тогда
			НовоеЗначение = Неопределено;
		ИначеЕсли ЭтоСериализацияРеквизита И ТолькоСсылка Тогда // Обход реквизитов основного объекта
			НовоеЗначение = СериализоватьПоСсылкеВСтруктуру(Значение, "СправочникСсылка", ТолькоСсылка, МетаданныеЗначения);
			
			Если НЕ ЭтотОбъектУжеОтсылался(Значение) Тогда
				ДополнитьСтекСсылокСледующегоУровня(Значение);
			КонецЕсли;
...
Функция СериализоватьПоСсылкеВСтруктуру(Знач ЗначениеСсылка, КоллекцияТипа, ТолькоСсылка = Ложь, МетаданныеЗначения)
	...
	Попытка
		...
		ЗначениеСтруктурой._Свойства.Вставить("Родитель",СериализоватьВПростыеТипы(ЗначениеРодитель, Истина, Истина));
		//СериализоватьВПростыеТипы(ЗначениеРодитель, Ложь);
	Исключение
	КонецПопытки;
	...
	Попытка
		...
		ЗначениеСтруктурой._Свойства.Вставить("Владелец",СериализоватьВПростыеТипы(ЗначениеРодитель, Истина, Истина));
		//СериализоватьВПростыеТипы(ЗначениеВладелец, Ложь);
	Исключение
	КонецПопытки;
...
Функция ВыгрузитьРеквизиты(ЗначениеСсылка, МетаданныеЗначения, СтруктураРеквизитов);
	...
	Для Каждого Реквизит Из мдРеквизиты Цикл
		...
		СсылкаСтруктурой = СериализоватьВПростыеТипы(ЗначениеРеквизита, Истина, Истина);
		
		//// Повторный вызов для полной сериализации реквизита объекта
...
СтекСсылкиКОбработке = Новый Соответствие;
УровеньВложенности = 0;
СериализацияЗакончена = Ложь;
