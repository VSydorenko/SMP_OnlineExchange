...
Процедура ПроизвестиЧтениеДанных(РезультирующаяСтрокаСИнформациейОбОшибке = "") Экспорт
	...
	Исключение
		...
		СтрокаОшибки = СтрШаблон("Ошибка при загрузке данных: %1", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция ПрочитатьОбъект(Знач ДанныеОбъекта)
	...
	МассивСвойств = мТекущийобъектДляЧтения.Получить("_Свойства");
	...
	Для Каждого ДанныеРеквизита Из МассивСвойств Цикл // ДанныеРеквизита - Соответствие, ключ - имя реквизита, значение - набор свойств объекта, или значение простого типа.
		...
		ИмяИсточник = ДанныеРеквизита.Ключ; // Это имя свойства в объекте-источнике;
		
		// Если нету ПКС - пропускаем свойство:
		СтрокаПКС = ТаблицаПКС.Найти(ИмяИсточник, "Источник");
		Если СтрокаПКС = Неопределено Тогда
			Продолжить;
		Иначе
			Имя = СтрокаПКС.Приемник; // Переопределяем. Это имя свойства уже в приемнике.
			ТипИсточникаСтрокой = СтрокаПКС.ТипИсточника;
			ТипПриемникаСтрокой = СтрокаПКС.ТипПриемника;
		КонецЕсли;
		
		// В КД2 эти 2 признака по-умолчанию определялись в ПКС и могли изменяться в обработчиках.
		// Так как у нас отсутствуют обработчики на стороне выгрузки - пока что берём из правил.
		НеЗамещатьСвойство = СтрокаПКС.НеЗамещать; //одАтрибут(ФайлОбмена, одТипБулево, "НеЗамещать"); // получать из ПКС
		ИмяПКО             = ?(Значениезаполнено(СтрокаПКС.ПравилоКонвертации), 
			СокрЛП(СтрокаПКС.ПравилоКонвертации.Код), ""); //одАтрибут(ФайлОбмена, одТипСтрока, "ИмяПКО"); // получаем из ПКС
		...
		Значение    = ПрочитатьСвойство(ТипСвойства, ИмяПКО, ДанныеРеквизита.Значение, ТипПриемникаСтрокой, ТипИсточникаСтрокой);
	...
	Если ТабЧасти <> Неопределено Тогда
		...
		Для Каждого ДанныеТЧ Из ТабЧасти Цикл
			...
			ИмяТч = ДанныеТЧ.Ключ;
			МассивПКГС = СтрокаПКО.Свойства.НайтиСтроки(Новый Структура("ЭтоГруппа, Источник, ВидИсточника", Истина, ИмяТч, "ТабличнаяЧасть"));
			Если МассивПКГС.Количество() = 0 Тогда
				Продолжить;
			Иначе
				стрПКГС = МассивПКГС[0];
			КонецЕсли;
...
Функция НайтиОбъектПоСсылке(ДанныеСсылки,
	...
	Пока НомерВариантаПоиска <= 10
		...
		Исключение
			...
			Сообщить("Последовательность полей поиска" + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ПрочитатьСвойстваПоискаИзФайла(
	...
	СвойстваТекущегоОбъекта = мТекущийобъектДляЧтения.Получить("_Свойства");
	Если СвойстваТекущегоОбъекта = Неопределено Тогда
	...
	Для Каждого стрПКС Из пксСвойстваПоиска Цикл
		...
		ПоискПоДатеНаРавенство = стрПКС.ПоискПоДатеНаРавенство;
		ИмяПКО = ?(стрПКС.ПравилоКонвертации <> Неопределено, стрПКС.ПравилоКонвертации.Имя, "");
		ЗначениеСвойства = ПрочитатьСвойство(ТипСвойства, ИмяПКО, ИмяИсточник);
...
// 
Функция ПрочитатьСвойство(Тип, ИмяПКО = "", ЗначениеИсточник, ТипПриемникаСтрокой = "", ТипИсточникаСтрокой = "")
	...
	Если ЭтоПростойТипДанных(Тип) И ЭтоПростойТипДанных(ТипЗначенияИсточник) Тогда
		
		Если Тип = ТипЗначенияИсточник Тогда // Идеальный случай - просто возвращаем значение.
			
			НаличиеСвойств = Истина;
			Значение = ЗначениеИсточник;
			
		ИначеЕсли Тип = одТипДата И ТипИсточникаСтрокой = "Дата" И ТипЗначенияИсточник = одТипСтрока Тогда // Для дат отдельный случай
			
			Значение = ПрочитатьДатуJSON(ЗначениеИсточник, ФорматДатыJSON.ISO);
			НаличиеСвойств = Истина;
			
		Иначе // Попытаемся привести значение
			
			НаличиеСвойств = Истина;
			Значение = одОписаниеТипа(Тип).ПривестиЗначение(ЗначениеИсточник);
			
		КонецЕсли;
		
	ИначеЕсли ТипЗначенияИсточник = одТипСоответствие Тогда // Может быть любая ссылка или Уникальный идентификатор или ДвоичныеДанные или другой сложный тип д.
		// to do: Доработать для случаев передачи двоичных данных и т.д.
		...
		ИначеЕсли СтрНайти(ТипСтрокой, "Ссылка") Тогда 
			Если ТипСтрокой = ТипИсточникаСтрокой Тогда // Это любая ссылка кроме перечисления и тип из соответствия совпадает с типом источника из правила
				
				Значение = НайтиОбъектПоСсылке(ЗначениеИсточник, Тип, ИмяПКО);
				НаличиеСвойств = Истина;
				
			ИначеЕсли НЕ ПустаяСтрока(ИмяПКО) И (ТипИсточникаСтрокой = "" ИЛИ ТипПриемникаСтрокой = "")  Тогда 
				// В КД2 если источник/приемник мог быть разных типов - "ТипИсточника"/"ТипПриемника" оставались пустыми.
				// Пока оставим так. В процессе отладки станет ясно как доработать.
		...
		Иначе
			...
			// to do: Здесь реализовать обработку иных типов данных
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	Если Не ПустаяСтрока(Конвертация.ПослеЗагрузкиДанных) Тогда
		...
		Исключение
			...
			Сообщить(ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + СтрШаблон(ИмяОбработчика, "ПослеЗагрузкиДанных"));
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, Имя, Очистить, ОбщаяИнформацияОТипеДокумента, НужноЗаписатьОбъект, 
	...
	МассивСтрокТЧ = ДанныеТЧ.Значение;
	НомерЗаписи = 0;
...
Процедура УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах,
	ПараметрыОбъекта, ИмяВетки, ДанныеПоискаПоТЧ, КопияТЧДляПоиска, НомерЗаписи, ДанныеСтрокиТЧ, ПравилаГруппы)
	...
	Для Каждого ДанныеРеквизитаТЧ Из ДанныеСтрокиТЧ Цикл
		
		ИмяИсточник = ДанныеРеквизитаТЧ.Ключ;
		стрПКС = ПравилаГруппы.Найти(ИмяИсточник, "Источник");
		
		Если стрПКС = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		ИмяПКО = ?(ЗначениеЗаполнено(стрПКС.ПравилоКонвертации), сокрЛП(стрПКС.ПравилоКонвертации.Код), "");
